...
fn consecutive_spares_each_get_a_one_roll_bonus() {
    ...
    let _ = game.roll(5); // 5  Open(5,0)  | Open(0,0)
    let _ = game.roll(5); // 10 Spare(5,5) | Open(0,0)
    let _ = game.roll(3); // 13 Open(3,0)  | Spare(5,5)
    let _ = game.roll(7); // 27 Spare(3,7) | Spare(5,5)
    let _ = game.roll(4); // 31 Open(4,0)  | Spare(3,7)
